...
Override with SBM_BOOK_STATUS="pilot,ready,scaffold" or SBM_BOOKS="slug1,slug2".

Usage:
    python build_books.py
    SBM_BOOKS_ROOT="W:/..." python build_books.py         # override source
    SBM_BOOK_STATUS="pilot,ready,scaffold" python build_books.py  # widen gate
"""
...
BOOKS_ROOT = Path(os.environ.get("SBM_BOOKS_ROOT", "W:/systems/books"))
...
PUBLISH_STATUS = {
    s.strip().lower()
    for s in os.environ.get("SBM_BOOK_STATUS", "pilot,ready").split(",")
    if s.strip()
}
_ALLOW = [s.strip() for s in os.environ.get("SBM_BOOKS", "").split(",") if s.strip()]
...
def discover_books() -> list[str]:
    ...
    for folder in sorted(BOOKS_ROOT.iterdir()):
        ...
        if _fm_str(meta, "status").lower() in PUBLISH_STATUS:
...
<title>{title} — Books — Structure Beats Magic</title>
...
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1"/>
<title>My library — Structure Beats Magic</title>
...
<main class="wrap">
  ...
  <h1>The books behind the method.</h1>
  <p class="subtitle" style="max-width:46rem">A curated shelf, not a reading list — each one earned its place. Here's why it's in my collection, the ideas worth stealing, and what I built with it. {count} books and counting.</p>
  {sections}
</main>
<footer><div class="wrap">Structure Beats Magic — a thesis by
  <a href="https://jacovanderlaan.com">Jaco van der Laan</a></div></footer>
